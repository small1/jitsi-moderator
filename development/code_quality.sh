...
lint_and_format() {
    ...
    print_header 'LINTER AND FORMAT HEALTH (MEGALINTER)'
    podman run --rm --volume "$(pwd)":/repo -e MEGALINTER_CONFIG='development/mega-linter.yml' -e DEFAULT_WORKSPACE=${MEGALINTER_DEF_WORKSPACE} -e LOG_LEVEL=INFO oxsecurity/megalinter-javascript:v7.7.0
...
commit() {
    ...
    if [[ "$(git rev-list --count ${compareToBranch}..)" == 0 ]]; then
        ...
        podman run --rm -i --volume "$(pwd)":/repo -w /repo ghcr.io/siderolabs/conform:v0.1.0-alpha.27-4-gecc1763 enforce --base-branch="${compareToBranch}"
...
coverage() {
    ...
    store_exit_code "$?" "Coverage" "${MISSING} ${RED}Coverage check failed, see logs (./coverage/index.html) and fix problems.${NC}\n" "${GREEN}${CHECKMARK}${CHECKMARK} Coverage check passed${NC}\n"
    printf '\n\n'
}
